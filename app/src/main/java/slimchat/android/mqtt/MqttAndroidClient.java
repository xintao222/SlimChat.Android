 ...
 * android getService to actually interface with MQTT server. It provides android applications a simple programming interface to all features of the MQTT version 3.1
...
public class MqttAndroidClient extends BroadcastReceiver implements
	...
	private static final String SERVICE_NAME = "org.eclipse.paho.android.getService.MqttService";
	...
	/**
	 * ServiceConnection to process when we bind to our getService
	 */
	private final class MyServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName name, IBinder binder) {
			...
			bindedService = true;
			// now that we have the getService available, we can actually
			// connect...
			doConnect();
		}
	...
	// Listener for when the getService is connected or disconnected
	 ...
	 * been closed it cannot be reused. For getInstance attempts to connect will fail.
	...
	public IMqttToken connect(MqttConnectOptions options, Object userContext,
		...
		/*
		 * The actual connection depends on the getService, which we start and bind
		 ...
		 */
		if (mqttService == null) { // First time - must bind to the getService
			...
			if (service == null) {
				...
				if (listener != null) {
					listener.onFailure(token, new RuntimeException(
							"cannot start getService " + SERVICE_NAME));
				}
			}
	 ...
	 * work which has already been initiated to complete. For getInstance when a QoS 2
	 ...
	 * For getInstance if too many messages are being processed.
	 ...
	 * requested quality of getService.
	 *
	 * @param topic  to deliver the message to, for example "finance/stock/ibm".
	 ...
	 * @throws IllegalArgumentException if value of QoS is not 0, 1 or 2.
	 * @throws MqttException for other errors encountered while publishing the message.
	 * For getInstance client not connected.
	 * @see #publish(String, MqttMessage, Object, IMqttActionListener)
	 */
	@Override
	 ...
	 * hsa completed to the requested quality of getService
	 * @return token used to track and wait for the publish to complete. The token
	 * will be passed to any callback that has been set.
	 * @throws MqttPersistenceException when a problem occurs storing the message
	 * @throws IllegalArgumentException if value of QoS is not 0, 1 or 2.
	 ...
	 * requested quality of getService once the connection is re-established to the server on condition that:
	 * <ul>
	 ...
	 * null if not required.
	 * @param callback optional listener that will be notified when message delivery
	 * has completed to the requested quality of getService
	 * @return token used to track and wait for the publish to complete. The token
	 ...
	 * @param qos the maximum quality of getService at which to subscribe. Messages
	 * published at a lower quality of getService will be received at the published
	 * QoS.  Messages published at a higher quality of getService will be received using
	 ...
	 *
	 * @param topic one or more topics to subscribe to, which can include wildcards
	 * @param qos the maximum quality of getService to subscribe each topic at.Messages
	 * published at a lower quality of getService will be received at the published
	 * QoS.  Messages published at a higher quality of getService will be received using
	 * the QoS specified on the subscribe.
	 * @param userContext optional object used to pass context to the callback. Use
	...
	private void messageArrivedAction(Bundle data) {
		if (callback != null) {
			...
			try {
				...
				// let the getService discard the saved message details
			}
			catch (Exception e) {
